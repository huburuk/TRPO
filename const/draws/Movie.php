    ...
    <div class="article">
      <div class="row">
        ...
        <div class="col-12 col-xl-8">
          ...
          ?><p style="margin-top:20px;" class="text_white"><?php echo $_SESSION['rs']; ?></p>
          <?php
          }
          ?>
          <video controls playsinline poster="../../uploads/thumbnail/<?php echo $thumbnail; ?>" id="player">
            <?php
            if ($GLOBALS['streaming'] == "1") {
              try {
                $conn = new PDO('mysql:host=' . DBHost . ';dbname=' . DBName . ';charset=' . DBCharset . ';collation=' . DBCollation . ';prefix=' . DBPrefix . '', DBUser, DBPass);
                ...
                $stmt = $conn->prepare("SELECT * FROM tbl_single_links WHERE item = ? AND streaming = '1' AND size <= ? ORDER BY size");
                $stmt->execute([$item_id, $max_vid_size]);
                $result = $stmt->fetchAll();

                foreach ($result as $row) {
            ?>
                  <source src="<?php echo "/../../../films/" . $row[2]; ?>" type="video/mp4" size="<?php echo $row[3]; ?>">
                <?php
                }
                ...
                foreach ($result as $row) {
                ?>
                  <track kind="captions" label="<?php echo $row[3]; ?>" srclang="<?php echo $row[4]; ?>" src="<?php echo $row[2]; ?>" <?php if ($sstart == "0") {
                                                                                                                                        print ' default ';
                                                                                                                                      } ?>>
            <?php
                  $sstart++;
                    ...
                    <tbody>
                      ...
                      ?>
                          <tr>
                            ...
                            ?>
                              <td>
                                <div class="main__table-text main__table-text--green"><a href="<?php echo $row[2]; ?>" download><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                      <path d="M21,14a1,1,0,0,0-1,1v4a1,1,0,0,1-1,1H5a1,1,0,0,1-1-1V15a1,1,0,0,0-2,0v4a3,3,0,0,0,3,3H19a3,3,0,0,0,3-3V15A1,1,0,0,0,21,14Zm-9.71,1.71a1,1,0,0,0,.33.21.94.94,0,0,0,.76,0,1,1,0,0,0,.33-.21l4-4a1,1,0,0,0-1.42-1.42L13,12.59V3a1,1,0,0,0-2,0v9.59l-2.29-2.3a1,1,0,1,0-1.42,1.42Z" />
                                    </svg> Download Now</a></div>
                              </td>
                            <?php
                            } else {
                            ?>
            ...
            if ($logged == "1") {
              if ($role == "user") {
                try {
                  ...
                  if (count($result) < 1) {
            ?>
                    <b id="wait"><button id="<?php echo $item_id; ?>" onclick="add_fav(this.id);" class="article__favorites" type="button">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                        ...
                        </svg> Add to favorites</button></b>
                  <?php
                  } else {
                  ?>
                    <button class='article__favorites' type='button'>Added to favorites</button>
              <?php
                  }
                } catch (PDOException $e) {
                  echo "Connection failed: " . $e->getMessage();
              ...
              ?>
              <a href="../../login?red=<?php echo $dlink; ?>" class="article__favorites" type="button"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                  <path d="M16,2H8A3,3,0,0,0,5,5V21a1,1,0,0,0,.5.87,1,1,0,0,0,1,0L12,18.69l5.5,3.18A1,1,0,0,0,18,22a1,1,0,0,0,.5-.13A1,1,0,0,0,19,21V5A3,3,0,0,0,16,2Zm1,17.27-4.5-2.6a1,1,0,0,0-1,0L7,19.27V5A1,1,0,0,1,8,4h8a1,1,0,0,1,1,1Z"></path>
        ...
        <div class="col-12 col-xl-8">
          <div class="categories">
            ...
            foreach ($genres as $el) {
            ?>
              <a href="../../genre/<?php echo strtolower($el); ?>" class="categories__item"><?php echo $el; ?></a>
            <?php
            }
            ?>
          </div>
          <div class="share">
            ...
            <a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo $dlink; ?>" class="share__link share__link--fb"><svg width="9" height="17" viewBox="0 0 9 17" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <a href="https://twitter.com/home?status=<?php echo $dlink; ?>" class="share__link share__link--tw"><svg width="16" height="12" viewBox="0 0 16 12" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <div class="tab-pane fade show active" id="tab-1" role="tabpanel">
                    ...
                    ?>
                      <li class="comments__item">
                        <div class="comments__autor">
                          ...
                          <span class="comments__name"><?php echo $row[11]; ?>
                            <?php if ($row[17] == "1") {
                              print '<img id="verified" title="Verified Account" class="verification" src="../../img/check.svg">';
                            } ?></span>
                ...
                <div class="row">
                  <div class="col-12" id="load_area" style="margin:auto">
                    <?php if ($pages > 12) {
                    ?>
                      <button id="loadBtn" class="catalog__more weeee">Load more</button>
                    <?php
                    } ?>
                  </div>
                ...
                ?><p class="text_white">Please <a href="../../login?red=<?php echo $dlink; ?>">Login</a> to comment</p>
                <?php
                }
                ?>
              </div>
              <div class="tab-pane fade" id="tab-2" role="tabpanel">
                      ...
                      ?>
                        <li class="reviews__item">
                          <div class="reviews__autor">
                            ...
                            <span class="reviews__name"><?php echo $row[3]; ?></span>
                            <span class="reviews__time"><?php echo $new_date; ?> by <?php echo $row[11]; ?>
                              <?php if ($row[17] == "1") {
                                print '<img id="verified" title="Verified Account" class="verification_small" src="../../img/check.svg">';
                              } ?></span>
                            <span class="reviews__rating"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M22,9.67A1,1,0,0,0,21.14,9l-5.69-.83L12.9,3a1,1,0,0,0-1.8,0L8.55,8.16,2.86,9a1,1,0,0,0-.81.68,1,1,0,0,0,.25,1l4.13,4-1,5.68A1,1,0,0,0,6.9,21.44L12,18.77l5.1,2.67a.93.93,0,0,0,.46.12,1,1,0,0,0,.59-.19,1,1,0,0,0,.4-1l-1-5.68,4.13-4A1,1,0,0,0,22,9.67Zm-6.15,4a1,1,0,0,0-.29.88l.72,4.2-3.76-2a1.06,1.06,0,0,0-.94,0l-3.76,2,.72-4.2a1,1,0,0,0-.29-.88l-3-3,4.21-.61a1,1,0,0,0,.76-.55L12,5.7l1.88,3.82a1,1,0,0,0,.76.55l4.21.61Z" />
                              </svg> <?php echo $rates; ?></span>
                ...
                <div class="row">
                  <div class="col-12" id="load_areab" style="margin:auto">
                    <?php if ($pagesb > 12) {
                    ?><button id="loadBtnb" class="catalog__more weeee">Load more</button>
                    <?php
                    } ?>
                  </div>
                ...
                if ($logged == "1") {
                  try {
                    ...
                    if (count($result) > 0) {
                ?>
                      <p class="text_white">You left a review on this item:</p>
                    <?php
                    } else {
                    ?>
                ...
                ?>
                  <p class="text_white">Please <a href="../../login?red=<?php echo $dlink; ?>">Login</a> to review</p>
                <?php
                }
                ?>
              </div>
